import { CRMService } from './src/support/crmService.js';
...
async function main() {
  const crmPath = process.env.CRM_PATH || './src/data/crm.json';
  const server = new CRMToolServer(crmPath);
  await server.start();

  process.on('SIGINT', () => {
    console.log('\n🛑 Shutting down CRM MCP Server...');
    process.exit(0);
  });
